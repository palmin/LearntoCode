...
func placeGemsOverTime() {
    ...
    gemRandomizer = RandomizedQueueObserver(randomRange: 0...5, world: world) { world in
        let existingGemCount = world.existingGems(at: world.coordinates(inColumns: [1,3], intersectingRows: [0,1,2])).count
        guard existingGemCount < 5 else { return }
        
        for coordinate in Set(world.coordinates(inColumns: [1,3], intersectingRows: [0,1,2])) {
