/*:
 **Goal:** Use the OR operator to adjust Byte's path if either of two conditions is true.
...
The last logical operator is the [logical OR operator (||)](glossary://logical%20OR%20operator), which combines two [Boolean](glossary://Boolean) conditions and runs your code if *at least* one is true. For example, in the following code, either `isOnGem` OR `isBlockedLeft` must be true.
 ...
 1. steps: Use the || operator to check whether one of two conditions is true. Hint: Byte may be blocked either in the front or on the left.
 2. If either is true, Byte should turn right and move forward.
 3. If neither is true, Byte should move forward.
*/
