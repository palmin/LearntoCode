...
public func assessmentPoint() -> AssessmentResults {
    ...
    var hints = [
        "When there is **not** a gem on the top platform, a staircase extends from that tile. Use the condition \"if Byte is NOT on a gem\" to determine what to do in that situation.",
    ...
    for element in contents {
        if !element.contains("!") && checker.didUseConditionalStatement {
            hints[0] = "Make sure that your condition looks like this: `!isOnGem`, with no spaces between the ! and `isOnGem`."
        }
