...
func placeItems() {
    world.placeGems(at: world.coordinates(inColumns: [6], intersectingRows: [3]))
    ...
    let platform2 = Platform(onLevel: 0, controlledBy: lock)
    world.place(platform2, at: Coordinate(column: 3, row: 4))
}

func placeBlocks() {
    world.removeNodes(at: world.coordinates(inColumns: [0,2,3], intersectingRows: 0...5))
