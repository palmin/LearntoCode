...
In this puzzle, a new function lets you move across multiple tiles using a single command, reducing the repetition in your code. Using a [parameter](glossary://parameter), you’ll specify an input (`distance`) for your function. When you [call](glossary://call) the function, you’ll pass in a value, or [argument](glossary://argument), for `distance`. For example, in `move(6)`, 6 is the argument.
