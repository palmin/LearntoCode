...
If Byte toggles a switch that’s already open, the switch will close. You’ll need to use an [``if`` statement](glossary://if%20statement) to check each switch before telling Byte to toggle it.

Use the new ``isOnClosedSwitch`` [command](glossary://command) as the **condition** in the ``if`` statement so you can tell Byte, "if is on closed switch, toggle switch."
