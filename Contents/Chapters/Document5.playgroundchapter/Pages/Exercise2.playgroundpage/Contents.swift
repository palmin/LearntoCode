...
To help Byte figure out whether to toggle a switch or collect a gem, use an [`if` statement](glossary://if%20statement) to check one possible condition and an [`else if` block](glossary://else%20if%20block) to check the other.

The new ``isOnGem`` [command](glossary://command) will help determine whether Byte is on a gem.
