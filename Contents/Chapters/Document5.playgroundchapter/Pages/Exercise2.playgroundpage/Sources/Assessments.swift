...
public func assessmentPoint() -> AssessmentResults {
    ...
    var hints = [
        "Start by moving Byte to the first random tile and use an `if` statement to check whether Byte is on a closed switch.",
        ...
        "If Byte is on a closed switch, toggle it. Else if Byte is on a gem, collect it.",

        ]
    
    if !checker.didUseConditionalStatement("if") {
        hints[0] = "First, add an `if` statement. Then tap on the word `if` in your code and tap \"Add 'else if' Statement\" to add an `else if` block to your code."
