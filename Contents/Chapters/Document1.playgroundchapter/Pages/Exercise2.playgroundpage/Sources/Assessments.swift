...
var hints = [
    ...
    if checker.functionCallCount(forName: "turnLeft") == 0 {
        ...
        hints[0] = "Oops! Every `moveForward()` command moves your character forward only one tile. To move forward two tiles, you need **two** `moveForward()` commands."
    } else if checker.calledFunctions == oneTileMovementNoCollect {
        hints[0] = "Oops! Every `moveForward()` command moves your character forward only one tile. You need **two** `moveForward()` commands to move forward two tiles. And don't forget to collect the gem!"
    }
