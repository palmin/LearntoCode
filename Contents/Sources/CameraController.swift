...
class CameraController {
    ...
    func performShortestFlyover(toFace direction: SCNFloat) -> Double {
        let deltaRot = angleBetweenCameraAnd(direction)
    ...
    func angleBetweenCameraAnd(_ direction: SCNFloat) -> SCNFloat {
        let delta = direction.capPhase() - rootHandle.rotation.w
    ...
    func zoomCamera(toYFov fov: Double, duration: Double = 1.0, completionHandler: (() -> Void)? = nil) {
        ...
        camera.yFov = max(min(fov, 55), 10.5)
    ...
    @objc func zoomCameraAction(_ recognizer: UIPinchGestureRecognizer) {
        ...
        let conversionFactor = M_PI / 180.0
        let angle = currentFOVy * conversionFactor
        let halfAngle = angle / 2.0
        let scaledHalfAngle = halfAngle / factor
        let a = 2.0 * atan(tan(scaledHalfAngle))
        let yFov = a / M_PI * 180.0
