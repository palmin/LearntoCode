...
public final class Block: WorldNode, LocationConstructible, NodeConstructible {
    ...
    lazy var texturesForDirection: [Direction: URL] = {
        ...
        for dir in Direction.all {
            let textureLevel = Block.levelFromName(for: self.node) % Block.patternCount
    ...
    public init() {
        ...
        baseNode.name = WorldNodeIdentifier.block.rawValue
    ...
    func addTop() {
        ...
        guard node.childNode(withName: Block.topName, recursively: false) == nil else { return }
    ...
    func addPatterenedGeo() {
        ...
        guard let wallsForLevel = Block.wallGeo(forLevel: level) else { fatalError() }
        wallsForLevel.eulerAngles.y = rotationDirection * rotationMagnitude
        node.addChildNode(wallsForLevel)
    }
    
    func removeHiddenQuads() {
        guard let world = world else { return }
        guard let wallsNode = node.childNodes.first else { return }
    ...
    func removeTop() {
        node.childNode(withName: Block.topName, recursively: true)?.removeFromParentNode()
    }
