...
public enum GridWorldSuccessState {
    case Success
    case Failure
}
...
@available(*, unavailable, message:"Move to GridWorldResults")
public typealias GroupedDiffResults = (correctResults: Set<DiffResult>, incorrectResults: Set<DiffResult>)
...
// This could also be part of the tuple: (incorrectResults: suboptimalResults: correctResults:)
// Could be used to color tiles differently. Tiles with `MoreThanOneItemFound` yellow.
enum ResultClassification {
    case correct
    case suboptimal // e.g. > 1 item placed
    case incorrect
}

// Also maybe something about the type of error that's more locked down than the description.
